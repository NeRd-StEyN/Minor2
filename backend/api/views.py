...
def twilio_test(request):
    # This TwiML dials the Vapi number automatically
    # Vapi number for the inbound assistant is +16088837889
    twiml = """<?xml version="1.0" encoding="UTF-8"?>
    <Response>
        <Say>Please wait while we connect you to Loke Mitra A.I.</Say>
        <Dial>+16088837889</Dial>
    </Response>"""
...
def start_outbound_calling(request):
    ...
    error_detail = call_response.get('error') if call_response else 'VAPI Outbound Call Failed'
    return Response({'success': False, 'error': error_detail}, status=500)
...
def start_inbound_agent(request):
    ...
    error_detail = agent_response.get('error') if agent_response else 'VAPI Inbound Agent Failed'
    return Response({'success': False, 'error': error_detail}, status=500)
...
def delete_document(request, file_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            doc.delete()
            print(f"Deleted {target_id} from Database.")
...
def delete_database(request):
    ...
    try:
        ...
        db_records.delete()
        print(f"Purged {count} record(s) with name '{db_name}' from local storage.")
