...
import { Edit2, Check, X, Plus, Phone, Upload, Database as DatabaseIcon, Settings, Info, PhoneOff, AlertCircle, UserPlus, Trash2, Zap } from 'lucide-react';
...
export default function HomePage({ userSession, accentColor, secondaryColor }: HomePageProps) {
  ...
  const [callingQueue, setCallingQueue] = useState<QueueEntry[]>([
    { id: '1', name: 'Nipun', phone: '+91 9075850416', notes: 'Test call for Nipun', status: 'pending' },
  ]);
    ...
    <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          {/* NEW: Talk to AI (Free) via Browser */}
          <motion.button
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-white shadow-lg text-sm sm:text-base transition-colors"
            ...
            onClick={() => {
              // @ts-ignore
              const VapiSDK = window.Vapi || (window as any).default?.Vapi;
              if (VapiSDK) {
                // @ts-ignore
                const vapi = (typeof VapiSDK === 'function') ? new VapiSDK("7ac2681b-7dfd-49e9-a960-b8c33dc2b810") : new (VapiSDK.default || VapiSDK)("7ac2681b-7dfd-49e9-a960-b8c33dc2b810");
                vapi.start("609e0891-4850-4492-bfcb-3625ab901263");
                console.log("Vapi Voice Session Started via Browser");
              } else {
                alert("Voice Brain is still initializing. Please wait 5 seconds and try again!");
              }
            }}
          >
            <Zap className="w-4 h-4 sm:w-5 sm:h-5" />
            <span>Talk to AI (FREE)</span>
          </motion.button>
          <Script
